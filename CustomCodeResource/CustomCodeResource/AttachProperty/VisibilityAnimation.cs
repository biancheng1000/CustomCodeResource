...
namespace CustomCode
{
    ...
    public class VisibilityAnimation
    {
        ...
        private static object CoerceVisibility(DependencyObject dependencyObject, object baseValue)
        {
            ...
            AnimationType animationType = GetAnimationType(frameworkElement);
            if (animationType == AnimationType.Fade)
            {
                return StarteDoubleAnimation(frameworkElement, visibility);
            }
            else
            {
                return  StarteThicknessAnimation(frameworkElement, visibility, animationType);
            }
            
        }

        private static object StarteDoubleAnimation(FrameworkElement frameworkElement, Visibility visibility)
        {
        ...
        private static object StarteThicknessAnimation(FrameworkElement frameworkElement, Visibility visibility,AnimationType animationType)
        {
            // If we get here, it means we have to start fade in or fade out animation. 
            // In any case return value of this method will be Visibility.Visible, to allow the animation.
            ...
            // When animation completes, set the visibility value to the requested value (baseValue)
            thicknessAnimation.Completed += (sender, eventArgs) =>
            {
                if (visibility == Visibility.Visible)
                {
                    // In case we change into Visibility.Visible, the correct value is already set
            ...
            if (visibility == Visibility.Collapsed || visibility == Visibility.Hidden)
            {
                if (animationType == AnimationType.CollapsedLeft)
                {
                    thicknessAnimation.From = new Thickness(0, 0, 0, 0);
                    thicknessAnimation.To = new Thickness(0, 0, -frameworkElement.ActualWidth, 0);
                }
                else if (animationType == AnimationType.CollapsedRight)
                {
                    thicknessAnimation.From = new Thickness(0, 0, 0, 0);
                    thicknessAnimation.To = new Thickness(-frameworkElement.ActualWidth, 0, 0, 0);
                }
                else if (animationType == AnimationType.CollapsedBottom)
                {
                    thicknessAnimation.From = new Thickness(0, 0, 0, 0);
                    thicknessAnimation.To = new Thickness(0, 0, 0, -frameworkElement.ActualHeight);
                }
                else if (animationType == AnimationType.CollapsedTop)
                {
                    thicknessAnimation.From = new Thickness(0, 0, 0, 0);
                    thicknessAnimation.To = new Thickness(0, -frameworkElement.ActualHeight,0,0);
                }
            }
            else
            {
                if (animationType == AnimationType.CollapsedLeft)
                {
                    ...
                    thicknessAnimation.From = new Thickness(0, 0, -frameworkElement.ActualWidth, 0);
                }
                else if (animationType == AnimationType.CollapsedRight)
                {
                    thicknessAnimation.To = new Thickness(0, 0, 0, 0);
                    thicknessAnimation.From = new Thickness(-frameworkElement.ActualWidth, 0, 0, 0);
                }
                else if (animationType == AnimationType.CollapsedBottom)
                {
                    thicknessAnimation.To = new Thickness(0, 0, 0, 0);
                    thicknessAnimation.From = new Thickness(0, 0, 0, -frameworkElement.ActualHeight);
                }
                else if (animationType == AnimationType.CollapsedTop)
                {
                    thicknessAnimation.To = new Thickness(0, 0, 0, 0);
                    thicknessAnimation.From = new Thickness(0, -frameworkElement.ActualHeight, 0, 0);
                }
            }
        ...
        private void AnimationCompleteHandel(object sender,EventArgs e)
        {
