...
namespace CustomCode
{
    ...
    public class LogText
    {
        ...
        //界面显示使用
        public static ObservableCollection<MemLog> showLog = new ObservableCollection<MemLog>();
        

        // 写日志文件线程的索引指针
        private static int _logSaveIndex = 0;
        ...
        public static void OnLogEvent(LogTextLevel level, string srcName, string logDescripton)
        {
            ...
            if(FunctionDic.Contains(functionName))
            {
                return;
            }
            ...
            {
                // 将日志写过程扔回UI线程，确保日志内存对象在多线程同时写时不冲突。
                // 在登录窗口直接关闭, 写出日志时, Application.Current为null
                //具有UI线程时，首先调用UI线程进行写入系统的缓存，如果没有UI线程，锁定堆栈，然后进行出栈操作
                if (Application.Current != null)
                {
                    Application.Current.Dispatcher.BeginInvoke(new Action(delegate()
                    {
                        ...
                        showLog.Insert(0, NewLog);

                        CheckOverMaxLogCount();
        ...
        private static void CheckOverMaxLogCount()
        {
            ...
            if (showLog.Count > 150)
            {   
                //在末尾减少日志
                showLog.RemoveAt(showLog.Count - 1);
            }
        ...
        private static void WriteThread()
        {
            ...
            {
                // 当前线程在日志队列写完后自动挂起
                if (_logSaveIndex >= _logMem.Count)
                {
                ...
                else
                {
                    try
                    {
                        MemLog LogToSave = _logMem[_logSaveIndex];

                        FileInfo fileInfo = new FileInfo(_logPath);

                      //  FileOperate.addpathPower(fileInfo.DirectoryName);
                        ...
                        //只显示错误的信息
                        //if (LogToSave.LogLevel == LogTextLevel.Error)
                        //{
                        //    Console.WriteLine("[{0}]<{1}>{2}", LogToSave.LogLevel.ToString(), LogToSave.LogSrcName, LogToSave.LogDesc);
                        //}
        ...
        public static void Info(string source, string format, params Object[] args)
        {
            ...
            OnLogEvent(LogTextLevel.Info, source, args==null||args.Length==0?format:string.Format(format, args));
        }
